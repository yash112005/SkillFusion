        ...
        {activeTab === 'analytics' && (
          <>
            ...
            {interviewHistory.length > 0 && (
              <section className="card p-0 overflow-hidden mt-8">
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-dark-card/50">
                  <h2 className="text-xl font-bold flex items-center text-gray-900 dark:text-white">
                    <History className="w-5 h-5 mr-2 text-primary-500" /> Recent Mock Interviews
                  </h2>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                      ...
                      {interviewHistory.slice(0, 5).map((item, i) => (
                        <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                          <td className="px-6 py-4 font-medium text-gray-900 dark:text-gray-200">
                            {item.role} <span className="text-xs text-gray-400">({item.level})</span>
                          </td>
                          <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{item.type}</td>
                          <td className="px-6 py-4 text-center font-black">
                            <span className={item.overallScore >= 80 ? 'text-green-500' : item.overallScore >= 60 ? 'text-yellow-500' : 'text-red-500'}>
                              {item.overallScore}%
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <Link 
                              to="/mock/interview" 
                              state={{ interviewData: item, step: 'summary' }}
